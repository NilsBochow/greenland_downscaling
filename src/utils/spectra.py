...
def mean_rapsd(data: xr.DataArray, normalize: bool=False):
    ...
    # Remove the assertion as we are allowing different dimensions
    lat_size = len(data.y)
    lon_size = len(data.x)
    print("len data", len(data))
    # Determine the size of the PSD based on the smaller dimension
    mean_psd = np.zeros(max(lat_size, lon_size) // 2)

    for i in tqdm(range(len(data))):
        data_slice = data[i].values
        if i<10: 
            plt.clf()
            plt.imshow(data_slice, vmin=-1, vmax=1, cmap="RdBu")
            plt.colorbar()
            plt.savefig(f"/p/projects/ou/labs/ai/Nils/arctic-downscaling/data/plots/mar_spectra/spectra_img_{i}.png")
        psd, freq = rapsd(data_slice, fft_method=np.fft, normalize=normalize, return_freq=True)
        if i<10: 
            plt.clf()
            plt.plot(freq, psd)
            plt.savefig(f"/p/projects/ou/labs/ai/Nils/arctic-downscaling/data/plots/mar_spectra/spectra_freq_{i}.png")
        mean_psd += psd
...
def mean_rapsd_numpy(data: np.ndarray, normalize: bool=False):
    ...
    for t in range(len(data)): #tqdm(range(200)):#
        data_slice = data[t]
        psd, freq = rapsd(data_slice, fft_method=np.fft, normalize=normalize, return_freq=True)
        mean_psd += psd
